...
# module's IAM outputs plus the placement-shape inputs below, and emits it as the
# `placement` output. The module writes nothing to the murmur catalog: the caller
# owns that write, which is what keeps this module to the AWS/TLS providers and
# lets one placement be managed however the caller already manages catalog state.
#
...
# null until placement_name is set, so this module still works for
# federation-only setups.
#
